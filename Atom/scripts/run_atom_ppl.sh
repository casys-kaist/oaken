...
export CUDA_VISIBLE_DEVICES=${GPU_NUM} # Avoid causing not same devices error in "auto" device_map.
...
echo "model,bit,wiki2" >> ${resultFile} #,ptb,c4,ptb-new,c4-new" >> ${resultFile}
echo ${MODEL},${BIT},${wiki2} >> ${resultFile} #,${ptb},${c4},${ptd_new},${c4_new} >> ${resultFile}
