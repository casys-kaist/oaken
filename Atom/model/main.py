...
if __name__ == '__main__':
    ...
    if args.reorder:            
        ...
        cached_file_path = f'{args.save_dir}/{model_name}_reorder_index_{args.dataset}.pt'
        if not os.path.isfile(cached_file_path):
            ...
            torch.save(reorder_index, cached_file_path)
        else:
            assert os.path.isfile(cached_file_path), "reorder index file not found."

            print("Loading cached reording index from disk...")
            reorder_index = torch.load(cached_file_path)
    ...
    if args.eval_common_sense:
        ...
        if args.multigpu:
            if ("llama" in args.model.lower()) or ("mixtral" in args.model.lower()) or ("mistral" in args.model.lower()):
        ...
        results = {}
        tasks_str = "piqa,hellaswag,winogrande"
