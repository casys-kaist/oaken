...
def get_model(model, seqlen, maxseqlen):
    ...
    # if model == "/home/ubuntu/model/llama2-70b/ckpt":
    #     print("DISABLE GQA")
    ...
    model = AutoModelForCausalLM.from_pretrained(model, config=config, trust_remote_code=True, use_flash_attention_2=True if model != "/home/ubuntu/model/llama2-70b/ckpt" else False, torch_dtype=torch.half)
...
@torch.no_grad()
def opt_calibration(model, dataloader, dev, perchannel_match, pertensor_match, bits, include_sparse=False, sparsity_threshold=0.999, nuq=False, fisher=None, norm=False, cap_outliers=False, first_few_fp16=False):
    print('OPT Calibration Starting ...')
    ...
    for i in range(len(layers)):
        ...
        for name in subset:
            ...
            if fisher is not None:
                key = 'model.decoder.layers.%d.%s' % (i, name)
                key = key + '.weight'
                fisher_info = fisher[key].cpu()
            else:
                fisher_info = None
            ...
            quantizers['model.decoder.layers.%d.%s' % (i, name)] = simquant[name].quantize(
                include_sparse=include_sparse,
                sparsity_threshold=sparsity_threshold,
                nuq=nuq,
                fisher=fisher_info,
