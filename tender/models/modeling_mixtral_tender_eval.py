...
# TODO @longjie no longer copied from Mistral after static cache
class MixtralAttention_Tender_Eval(nn.Module):
    """
    ...
    def forward(
        ...
        # query_states = Q.reshape(padded_rows, -1)[:orig_tgt_len * bsz]
        # K = torch.matmul(result, self.WK) + k_ch_bias 
        # key_states = K.reshape(padded_rows, -1)[:orig_tgt_len * bsz]
        # V = torch.matmul(result, self.WV) + v_ch_bias 
        # value_states = V.reshape(padded_rows, -1)[:orig_tgt_len * bsz]

        # if self.quant_out_bf16:
        #     query_states = quant_bfloat(query_states)
        #     key_states = quant_bfloat(key_states)
        #     value_states = quant_bfloat(value_states)
        #========================================
        ...
        #print(count)
        #print(sum(count), self.embed_dim, chunks)
...
# TODO @longjie no longer copied from Mistral after static cache
class MixtralFlashAttention2(MixtralAttention_Tender_Eval):
    """
    Mixtral flash attention module. This module inherits from `MixtralAttention_Tender_Eval` as the weights of the module stays
...
# TODO @longjie no longer copied from Mistral after static cache
class MixtralSdpaAttention(MixtralAttention_Tender_Eval):
    """
    Mixtral attention module using torch.nn.functional.scaled_dot_product_attention. This module inherits from
    `MixtralAttention_Tender_Eval` as the weights of the module stays untouched. The only changes are on the forward pass to adapt to
    SDPA API.
    """

    # Adapted from MixtralAttention_Tender_Eval.forward
    def forward(
        self,
        hidden_states: torch.Tensor,
        attention_mask: Optional[torch.Tensor] = None,
...
MIXTRAL_ATTENTION_CLASSES = {
    "eager": MixtralAttention_Tender_Eval,
...
class MixtralSparseMoeBlock(nn.Module):
    ...
    def __init__(self, config, layer_idx: int):
        ...
        self.experts = nn.ModuleList([MixtralBlockSparseTop2MLP(config, self.layer_idx) for _ in range(self.num_experts)])
