...
def build_model_and_tokenizer(model_name, seq_len):
    if os.path.exists('../../models/modeling_mistral.py'):
        os.system('rm ../../models/modeling_mistral.py')
    cwd = os.getcwd()
    os.chdir('../../models/')
    os.system('ln -s modeling_mistral_tender.py modeling_mistral.py')
    os.chdir(cwd)
    ...
    tokenizer = AutoTokenizer.from_pretrained(f"mistralai/Mistral-7B-v0.3", use_fast=False, model_max_length=seq_len)
