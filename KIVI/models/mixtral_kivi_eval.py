...
# TODO @longjie no longer copied from Mistral after static cache
class MixtralAttention_KIVI_eval(nn.Module):
    """
...
# TODO @longjie no longer copied from Mistral after static cache
class MixtralFlashAttention2(MixtralAttention_KIVI_eval):
    """
    Mixtral flash attention module. This module inherits from `MixtralAttention_KIVI` as the weights of the module stays
...
# TODO @longjie no longer copied from Mistral after static cache
class MixtralSdpaAttention(MixtralAttention_KIVI_eval):
    """
    Mixtral attention module using torch.nn.functional.scaled_dot_product_attention. This module inherits from
    `MixtralAttention_KIVI` as the weights of the module stays untouched. The only changes are on the forward pass to adapt to
    SDPA API.
    """

    # Adapted from MixtralAttention_KIVI.forward
    def forward(
        self,
        hidden_states: torch.Tensor,
        attention_mask: Optional[torch.Tensor] = None,
...
MIXTRAL_ATTENTION_CLASSES = {
    "eager": MixtralAttention_KIVI_eval,
...
class MixtralDecoderLayer_KIVI_eval(nn.Module):
...
class MixtralPreTrainedModel(PreTrainedModel):
    ...
    _no_split_modules = ["MixtralDecoderLayer_KIVI_eval"]
...
# TODO @longjie no longer copied from Mistral after static cache
class MixtralModel_KIVI_eval(MixtralPreTrainedModel):
    """
    ...
    def __init__(self, config: MixtralConfig):
        ...
        print("This is modified MixtralModel_KIVI class")
        self.padding_idx = config.pad_token_id
        self.vocab_size = config.vocab_size

        self.embed_tokens = nn.Embedding(config.vocab_size, config.hidden_size, self.padding_idx)
        self.layers = nn.ModuleList(
            [MixtralDecoderLayer_KIVI_eval(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
...
class MixtralForCausalLM_KIVI_eval(MixtralPreTrainedModel, GenerationMixin):
    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, config):
        super().__init__(config)
        self.model = MixtralModel_KIVI_eval(config)
        self.vocab_size = config.vocab_size
