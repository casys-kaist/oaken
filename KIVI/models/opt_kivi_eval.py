...
class OPTAttention_KIVI_eval(nn.Module):
    ...
    def forward(
        ...
        if past_key_value is not None:
            ...
            # Calculate attn_output from dequantized value_states
            value_full_length = value_states_full.shape[-2]
            if value_states_quant is None:
                attn_output = torch.matmul(attn_weights, value_states_full)
            else:
...
class OptFlashAttention2(OPTAttention_KIVI_eval):
    """
...
class OPTSdpaAttention(OPTAttention_KIVI_eval):
    """
...
OPT_ATTENTION_CLASSES = {
    "eager": OPTAttention_KIVI_eval,
    "flash_attention_2": OPTAttention_KIVI_eval,
    "sdpa": OPTAttention_KIVI_eval,
}


class OPTDecoderLayer_KIVI_eval(nn.Module):
...
class OPTDecoder_KIVI_eval(OPTPreTrainedModel):
    """
    ...
    def __init__(self, config: OPTConfig):
        ...
        self.layers = nn.ModuleList([OPTDecoderLayer_KIVI_eval(config) for _ in range(config.num_hidden_layers)])
...
class OPTModel_KIVI_eval(OPTPreTrainedModel):
    def __init__(self, config: OPTConfig):
        super().__init__(config)
        print("This is OPTModel_KIVI class modified for evaluation")
        self.decoder = OPTDecoder_KIVI_eval(config)
        # Initialize weights and apply final processing
        self.post_init()
...
class OPTForCausalLM_KIVI_eval(OPTPreTrainedModel, GenerationMixin):
    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, config):
        super().__init__(config)
        self.model = OPTModel_KIVI_eval(config)
...
class OPTForSequenceClassification_KIVI_eval(OPTPreTrainedModel):
    def __init__(self, config: OPTConfig):
        super().__init__(config)
...
class OPTForQuestionAnswering_KIVI_eval(OPTPreTrainedModel):
    def __init__(self, config: OPTConfig):
        super().__init__(config)
        self.model = OPTModel_KIVI_eval(config)
