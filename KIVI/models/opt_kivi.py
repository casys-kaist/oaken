...
class OPTAttention_KIVI(nn.Module):
...
class OptFlashAttention2(OPTAttention_KIVI):
    """
...
OPT_ATTENTION_CLASSES = {
    "eager": OPTAttention_KIVI,
    "flash_attention_2": OptFlashAttention2,
    "sdpa": OPTSdpaAttention,
}


class OPTDecoderLayer_KIVI(nn.Module):
...
class OPTDecoder_KIVI(OPTPreTrainedModel):
    """
    ...
    def __init__(self, config: OPTConfig):
        ...
        self.layers = nn.ModuleList([OPTDecoderLayer_KIVI(config) for _ in range(config.num_hidden_layers)])
...
class OPTModel_KIVI(OPTPreTrainedModel):
    def __init__(self, config: OPTConfig):
        super().__init__(config)
        self.decoder = OPTDecoder_KIVI(config)
        # Initialize weights and apply final processing
        self.post_init()
...
class OPTForCausalLM_KIVI(OPTPreTrainedModel, GenerationMixin):
    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, config):
        super().__init__(config)
        self.model = OPTModel_KIVI(config)
